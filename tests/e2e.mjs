...
console.log('\n1. EN only');
{
  ...
  const shortEN = await page.evaluate(() => buildShortSummaryHTML());
  check('short (EN only): participation, engagement, period only', ['Participation', 'Engagement Quality', 'Performance by Period'].every(h => shortEN.includes(`>${h}<`)) && !shortEN.includes('(GA4)') && shortEN.includes('<strong>Date Range:</strong>'));
  check('two summary buttons', await page.evaluate(() => !!document.getElementById('btn-copy-short') && document.getElementById('btn-copy-summary').textContent.includes('Copy Full Summary')));
...
console.log('\n2. EN + basic GA4 dropped together');
{
  ...
  const shortHtml = await page.evaluate(() => buildShortSummaryHTML());
  const order = ['Participation', 'Engagement Quality', 'Entry Page Type (GA4)', 'Top Ladder Entry Pages (GA4)', 'Opt-Ins Taken (GA4)', 'First Step Shown (GA4)', 'Performance by Period'].map(h => shortHtml.indexOf(`<strong>${h}</strong>`));
  check('short: section order', order.every((v, i) => v >= 0 && (i === 0 || v > order[i - 1])), order.join(','));
  check('short: internal sections dropped', ['Traffic Source Mix', 'Device Breakdown', 'GA4 Tracking Coverage', 'Ladder Position Averages', 'Depth Distribution', 'Generated', 'Opt-In Ladder Performance Summary', 'Outliers excluded', '<strong>Ladder Entry Pages (GA4)</strong>', '<strong>GA4:</strong>'].every(t => !shortHtml.includes(t)));
  check('short: header lines', shortHtml.includes('<strong>Date Range:</strong>') && shortHtml.includes('<strong>View:</strong> Monthly'));
  check('short: 10% trim on opt-ins with footnote', shortHtml.includes('Newsletter') && shortHtml.includes('Action Alerts') && !shortHtml.includes('Wildlife Updates') && !shortHtml.includes('Text Messages') && shortHtml.includes('(2 of 4)'));
  check('short: all page types kept', shortHtml.includes('>Advocacy<') && shortHtml.includes('>Donation<'));
  check('short: % of Visitors in opt-in and first-step tables only', (shortHtml.match(/% of Visitors<\/th>/g) || []).length === 2);
  const shortTxt = await page.evaluate(() => buildShortSummaryPlaintext());
  check('short: plaintext twin', shortTxt.startsWith('Date Range: ') && shortTxt.includes('PERFORMANCE BY PERIOD') && shortTxt.includes('of visitors') && !shortTxt.includes('TRAFFIC SOURCE') && !shortTxt.includes('Wildlife Updates'));
  await page.selectOption('#filter-period', 'm:2025-03'); await page.waitForTimeout(100);
  check('short: long month header', (await page.evaluate(() => buildShortSummaryHTML())).includes('<strong>Date Range:</strong> March 2025'));
  await page.selectOption('#filter-period', 'q:2025-Q1'); await page.waitForTimeout(100);
  check('short: quarter header', (await page.evaluate(() => buildShortSummaryHTML())).includes('<strong>Date Range:</strong> Q1 2025'));
...
for (const [file, assert] of edge) {
  ...
  if (file === 'ga4-tsv-meta.tsv') {
    ...
    const shortTsv = await page.evaluate(() => buildShortSummaryHTML());
    check('tsv: short summary hides (not set) page and explains it', !shortTsv.includes('(not set)') && shortTsv.includes('had no recorded entry page'));
  }
